...
export default function AlbumPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden">
      {/* Full-bleed hero with blurred cover background */}
        ...
        <div className="relative h-full flex flex-col justify-end px-6 sm:px-10 lg:px-16 pb-8 pt-20">
          <div className="flex flex-col sm:flex-row items-end gap-8">
            ...
            <div className="relative flex-shrink-0 group">
              <div className="absolute -inset-1 bg-musify-teal/30 rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition" />
              <img
                src={coverImage}
                alt={displayAlbum}
            ...
            <div className="flex-1 min-w-0">
              ...
              <Link
                href={`/album?artist=${encodeURIComponent(displayArtist)}`}
                className="inline-flex items-center gap-3 mt-4 group/artist"
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-musify-teal to-musify-purple flex items-center justify-center text-white font-bold text-sm shadow-lg">
                  {displayArtist.charAt(0)}
                </div>
                <span className="text-white/90 font-semibold group-hover/artist:text-white transition">
                  {displayArtist}
                </span>
              </Link>
              <div className="flex flex-wrap items-center gap-3 mt-4 text-white/60 text-sm">
                {releaseYear && <span>{releaseYear}</span>}
                {releaseYear && <span className="w-1 h-1 rounded-full bg-white/40" />}
              ...
              <div className="flex items-center gap-4 mt-6">
                <button
                  onClick={handlePlay}
                  className="w-16 h-16 rounded-full bg-musify-teal hover:scale-105 active:scale-95 flex items-center justify-center shadow-lg shadow-musify-teal/40 transition-all duration-200"
                >
